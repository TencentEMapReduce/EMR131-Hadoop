  ...
  protected void renderData(Block html) {
    TBODY<TABLE<Hamlet>> tbody =
        html.table("#apps").thead().tr().th(".id", "ID").th(".user", "User")
          ...
          .th(".runningContainers", "Running Containers")
    ...
    for (ApplicationReport appReport : appReports) {
      ...
      Set<String> nodes = RMAppAttemptBlock.getBlacklistedNodes(rm, appAttemptId);
      if (nodes != null) {
      ...
      appsTableData
        ...
        .append("\",\"")
        .append(app.getRunningContainers() == -1 ? "0" : String
