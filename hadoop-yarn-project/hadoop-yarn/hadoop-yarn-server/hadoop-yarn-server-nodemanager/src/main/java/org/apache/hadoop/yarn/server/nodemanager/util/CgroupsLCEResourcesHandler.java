...
public class CgroupsLCEResourcesHandler implements LCEResourcesHandler {
  ...
  public String getResourcesOption(ContainerId containerId) {
    ...
    if (sb.charAt(sb.length() - 1) == LinuxContainerExecutor.LINUX_FILE_PATH_SEPARATOR) {
  ...
  private String findControllerInMtab(String controller,
    ...
    for (Entry<String, List<String>> e : entries.entrySet()) {
      if (e.getValue().contains(controller)) {
        ...
        // controller name leads to container launch failure. Symlinks 'cpu' and
        // 'cpuacct' point to 'cpu,cpuacct'. Using 'cpu' solves the issue
