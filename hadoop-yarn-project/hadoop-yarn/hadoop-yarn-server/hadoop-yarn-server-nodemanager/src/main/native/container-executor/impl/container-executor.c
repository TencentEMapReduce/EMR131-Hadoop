...
int launch_container_as_user_systemd(const char *user, const char *app_id,
                   const char *container_id, const char *work_dir,
                   const char *script_name, const char *cred_file,
                   const char* pid_file, char* const* local_dirs,
                   char* const* log_dirs, const char *resources_key,
                   const char *resources_value) {
  int exit_code = -1;
  ...
  // Change to root user
  if (change_effective_user(0, 0) != 0) {
    fprintf(LOGFILE, "Can't change to root user, %s\n", strerror(errno));
    goto cleanup;
  }
  ...
  commands[0] = "sh";
  commands[1] = "-c";
  int size = 55 + strlen(resources_value) + strlen(user) + strlen(script_file_dest);
  commands[2] = malloc(size);
  memset(commands[2], '\0', size);
  strcat(commands[2], "systemd-run --scope --slice ");
  strcat(commands[2], resources_value);
  strcat(commands[2], " /usr/sbin/runuser -u ");
  strcat(commands[2], user);
  strcat(commands[2], " sh ");
  strcat(commands[2], script_file_dest);
  commands[3] = NULL;
